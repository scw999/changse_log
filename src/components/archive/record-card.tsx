...
export function RecordCard({ record, compact = false }: Readonly<RecordCardProps>) {
  ...
  return (
    ...
    >
      <div className={cx("pointer-events-none absolute inset-x-0 top-0 h-28 bg-gradient-to-br opacity-80", meta.accentClass)} />
      <div className="relative">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[11px] uppercase tracking-[0.3em] text-stone-500">{getRecordMetaLine(record)}</p>
            <h3 className={cx("mt-3 font-display leading-tight text-stone-950", compact ? "text-xl" : "text-2xl")}>
              {record.title}
            </h3>
          </div>
